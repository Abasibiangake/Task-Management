...
function List(props) {
  ...
  const deleteTask = (item) => {
    ...
    const updateTask = (item) => {
        setShowLoading(true);
        const id = item._id;
        const data = { taskId: item.taskId, taskName: item.taskName, 
            taskDescription: item.taskDescription,startDate: item.startDate, endDate: item.endDate,
          owner: item.owner };
          console.log('task to update:', data)
        const apiUrlUpdate = "http://localhost:3000/tasks/" + id;
        console.log('url:',apiUrlUpdate)

        axios.get(apiUrlUpdate, data)
          .then((results) => {
            setShowLoading(false);
            console.log('updated document:', results.data)

            fetchData()

            //navigate('/updateList')
           navigate('/updateList/'+id, { state: { id: item._id} });
          })
          .catch((error) => setShowLoading(false));
      };
  //show the task
          ...
          <ListGroup>
            ...
            <tbody>
              {data.map((item, idx) => (
                  ...
                  <td><Button type="button" variant="danger" onClick={() => { updateTask(item) }}>Update</Button></td>
